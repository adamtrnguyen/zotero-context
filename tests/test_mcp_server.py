...
@pytest.fixture()
def wired(zotero, linker, cookjohn, monkeypatch, tmp_path):
    """Substitute the fakes for the classes every module constructs.

    The verbs resolve their transports through the module-global name, so patching the
...
def test_a_blocked_write_renders_as_structured_data_with_a_stable_code(wired):
    ...
    with pytest.raises(WriteBlocked) as caught:
        adapter.call_writes("zotero_set_tags", {"item_key": "ABCD2345", "tags": ["new"]})
...
def test_every_reason_code_the_adapter_can_emit_is_in_the_published_set(wired):
    ...
    for tool, arguments in (
        ...
        with pytest.raises(WriteBlocked) as caught:
            adapter.call_writes(tool, arguments)
...
def test_replacing_creators_through_mcp_still_needs_the_second_ask(wired):
...
def test_force_passed_through_mcp_completes_the_replacement(wired):
...
def test_a_no_op_update_is_still_refused_through_mcp(wired):
...
def test_a_batch_with_one_unresolvable_key_is_refused_whole(wired):
    """The gate that motivated the package: the plugin would trash the resolvable ones
    and return success."""
    wired.add("ABCD2345", "A Paper")
    with pytest.raises(WriteBlocked) as caught:
        adapter.call_writes("zotero_trash_items", {"item_keys": ["ABCD2345", "NOSUCH12"]})
    assert caught.value.code == "unknown_item_keys"


def test_trash_and_restore_round_trip_through_mcp(wired, linker):
    """The verb that had no MCP tool at all, end to end over the adapter."""
    wired.add("ABCD2345", "A Paper")
    trashed = adapter.call_writes("zotero_trash_items", {"item_keys": ["ABCD2345"]})
    assert trashed["ok"] is True
    assert trashed["transport"] == "linker"
    assert wired.is_trashed("ABCD2345") is True

    restored = adapter.call_writes("zotero_restore_items", {"item_keys": ["ABCD2345"]})
...
def test_an_unknown_tool_is_named_in_the_error(wired):
    with pytest.raises(ValueError, match="Unknown tool: zotero_nope"):
        adapter.call_writes("zotero_nope", {})


def test_an_undeclared_argument_is_refused_rather_than_dropped(wired):
...
def test_an_absent_required_argument_is_named(wired):
    with pytest.raises(ValueError, match=r"requires \['item_keys'\]"):
        adapter.call_writes("zotero_trash_items", {})


def test_an_explicit_null_on_an_optional_argument_falls_back_to_the_default(wired):
...
def test_a_null_on_a_required_argument_reaches_the_verbs_coded_gate(wired):
    ...
    with pytest.raises(WriteBlocked) as caught:
        adapter.call_writes("zotero_trash_items", {"item_keys": None})
...
def test_preflight_reports_each_transport_separately(wired, monkeypatch):
